...
fn migrate(connection: &mut Connection) -> Result<(), StoreError> {
    ...
    // before this was.
    //
    // Asked of the table rather than of `user_version`, because this column
    // and the one above it were written on two branches that both numbered
    // themselves 5. Libraries are already out there stamped with that number
    // and holding either column, so the number cannot say which one is
    // missing — only the table can.
