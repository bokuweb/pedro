...
- **The model is never asked which book it is quoting.** A source is resolved by
  looking its quotation up in each book under the shelf until one holds it. A
  title the model copies slightly wrong would cost the reader the jump; a
  quotation it copies slightly wrong is already handled, by the fragment
  matching the citation lookup has always done.
